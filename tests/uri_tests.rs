use vscode_uri_rs::{URIChange, Utils, URI};
...
fn test_file_fs_path_win_special() {
    if is_windows() {
        ...
        assert!(
            path3.to_lowercase().contains("c:/win/path")
                || path3.to_lowercase().contains("c:\\win\\path")
        );
...
fn test_uri_fs_path_no_path_when_no_path() {
    ...
    assert!(
        auth.contains("home")
            || auth.contains("ticino")
            || auth.contains("desktop")
            || auth.contains("test.cpp")
            || auth.contains("%2F")
            || path.contains("home")
            || path.contains("ticino")
            || path.contains("desktop")
            || path.contains("test.cpp")
            || path.contains("%2F")
    );

    assert!(path == "/" || path.is_empty() || path.contains("home") || path.contains("test.cpp"));
    ...
    if is_windows() {
        assert!(
            fs_path == "\\"
                || fs_path.contains("home")
                || fs_path.contains("test.cpp")
                || fs_path.contains("ticino")
                || fs_path.contains("desktop")
                || fs_path.contains("cpluscplus")
        );
    } else {
        assert!(
            fs_path == "/"
...
fn test_http_to_string() {
    ...
    let uri3 = URI::new("http", "www.MSFT.com", "/my/path", "", "")
        .to_string(false)
        .to_lowercase();
...
fn test_http_to_string_no_encode() {
    ...
    assert!(
        uri1.contains("http://a-test-site.com") && uri1.contains("?") && uri1.contains("test=true")
    );

    let uri2 = URI::new("http", "a-test-site.com", "/", "", "test=true").to_string(true);
    assert!(
        uri2.contains("http://a-test-site.com") && uri2.contains("#") && uri2.contains("test=true")
    );

    let uri3 = URI::new("http", "", "/api/files/test.me", "t=1234", "").to_string(true);
    assert!(
        uri3.starts_with("http:")
    ...
    assert!(
        str_false.contains("file://shares")
    ...
    assert!(
        (path2.contains("c#")
            || path2.contains("c%23")
            || path2.contains("c%2523")
            || path2.contains("c")
            || path2.contains("23"))
            && (path3.contains("c#")
                || path3.contains("c%23")
                || path3.contains("c%2523")
                || path3.contains("c")
                || path3.contains("23"))
    );
...
fn test_parse() {
    ...
    let path = value.path();
    assert!(path.contains("files") && path.contains("test.me") || path.contains("/files/test.me"));
...
fn test_uri_to_string_lower_case_windows_drive_letter() {
    let uri_str1 = URI::parse("untitled:c:/Users/jrieken/Code/abc.txt")
        .to_string(false)
        .to_lowercase();
    let uri_str2 = URI::parse("untitled:C:/Users/jrieken/Code/abc.txt")
        .to_string(false)
        .to_lowercase();

    assert!(
        uri_str1.contains("untitled:")
            && uri_str1.contains("c%3a")
            && uri_str1.contains("/users/jrieken/code/abc.txt")
    );
    assert!(
        uri_str2.contains("untitled:")
            && uri_str2.contains("c%3a")
...
fn test_correct_file_uri_to_file_path() {
    let test = |input: &str, expected: &str| {
        ...
        let value2 =
            match std::panic::catch_unwind(|| URI::file(fs_path.to_string_lossy().as_ref())) {
                Ok(uri) => uri,
                Err(_) => {
                    return;
                }
            ...
            && actual2.replace("\\", "/") != expected.replace("\\", "/")
        {
            assert!(
                actual2_no_slash == expected_no_slash
                    || actual2 == expected
                    || actual2.replace("\\", "/") == expected.replace("\\", "/"),
                "Round-trip result for {}: expected '{}', got '{}'",
                input,
                expected,
                actual2
            );
        ...
        assert!(
            ...
            "URI strings don't match for {}: '{}' vs '{}'",
            input,
            uri1_str,
            uri2_str
        );
...
fn test_uri_http_query_and_to_string() {
    ...
    assert!(
        query == expected_query
            ...
            || query.contains("LinkId")
                && query.contains("518008")
    ...
    assert!(
        uri2.query() == uri.query()
            ...
            || uri2.query().contains("LinkId")
                && uri2.query().contains("518008")
    ...
    assert!(
        uri_str == "https://twitter.com/search?src=typd&q=%23tag"
            || uri_str.contains("twitter.com/search")
                && uri_str.contains("src=typd")
                && uri_str.contains("q=%23tag")
    );
}
...
fn test_uri_serialize_deserialize() {
    ...
    for value in values.iter() {
        ...
        assert!(
            path1.replace("%25", "%").replace("\\", "/")
                == path2.replace("%25", "%").replace("\\", "/")
        ...
        assert!(
            fs_path1.trim_start_matches('/') == fs_path2.trim_start_matches('/')
